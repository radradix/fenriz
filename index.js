...
bot.on('message', async message => {
    ...
    const pyshell = new py.PythonShell('interactive_conditional_samples.py', options);  
        ...
        //new Promise(fulfill => {  
            ...
            pyshell.stdout.on('data', function (data) {  
                ...
                if(data.includes("<|endoftext|>")){
                    return message.channel.send(data.substring(0,data.indexOf("<|endoftext|>")))
                } else return message.channel.send(data.substring(0,1000));
            }); 
